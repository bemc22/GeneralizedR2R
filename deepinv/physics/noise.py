...
class GammaNoise(torch.nn.Module):
    ...
    def forward(self, x, l=None, **kwargs):
        ...
        concentration = self.l.to(x.device)
        rate = concentration # / ( x + 1e-6 )
        rate = rate.to(x.device)

        d = torch.distributions.gamma.Gamma(concentration, rate ) # previous self.l / x
        shape = x.shape
        return d.sample(shape) * x  
