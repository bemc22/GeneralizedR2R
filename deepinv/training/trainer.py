...
class Trainer:
    ...
    def prepare_images(self, physics_cur, x, y, x_net):
        ...
        # y     = torch.log(7*y + 1) / torch.log(torch.tensor(8.0))
        ...
        with torch.no_grad():
            ...
            if vis_array.shape[1] == 2:
                vis_array = (
                    vis_array.pow(2).sum(1).sqrt().unsqueeze(1).repeat(1, 3, 1, 1)
                )

            for i in range(len(vis_array)):
                vis_array[i] = rescale_img(vis_array[i], rescale_mode="clip")
    ...
    def save_model(self, epoch, eval_metrics=None, state={}):
        ...
        if (epoch > 0 and epoch % self.ckp_interval == 0) or epoch + 1 == self.epochs:
            ...
            torch.save(
                state,
                os.path.join(
                    Path(self.save_path), Path("ckp_last.pth.tar")
                ),
            )
