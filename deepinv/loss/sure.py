...
class SureGammaModel(torch.nn.Module):
    ...
    def forward(self, y, physics, update_parameters=False):
        ...
        out = self.model(y, physics)
        out = out.clamp(0, 1)
        # replace values in abs(y) < 1e-5 with 1e-5
...
class SureGammaLoss(Loss):
    ...
    def __init__(self, metric=torch.nn.MSELoss(), l=1.0):
    ...
    def forward(self, x_net, y, physics, model, **kwargs):
        ...
        div    = 2 *  mc_div(y1, target, model, physics, 1e-2) * 0.001

        return self.metric(y1, target) + div

    def adapt_model(self, model):

        if isinstance(model, SureGammaModel):
            return model
        else:
