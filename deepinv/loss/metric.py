...
class SSIM(Loss):
    ...
    def __init__(self, multiscale=False, train=False, device="cpu", log=False):
        super().__init__()
        check_pyiqa()
        if multiscale:
    ...
    def forward(self, x, x_net, **kwargs):
        ...
        :return: torch.Tensor size (batch_size,).
        """
        if self.log: # compute on log scale
            x     = torch.log(7*x + 1)     / torch.log(torch.tensor(8.0))
            x_net = torch.log(7*x_net + 1) / torch.log(torch.tensor(8.0))
...
class PSNR(Loss):
    ...
    def __init__(self, max_pixel=1, normalize=False, log=False):
    ...
    def forward(self, x_net, x, **kwargs):
        ...
        if self.log: # compute on log scale
            scale = 1024
            x     = torch.log(scale*x + 1)     / torch.log(torch.tensor(scale+1))
            x_net = torch.log(scale*x_net + 1) / torch.log(torch.tensor(scale+1))

        if x.shape[1] == 2: # complex image
            x     = x.pow(2).sum(1).sqrt().unsqueeze(1)
            x_net = x_net.pow(2).sum(1).sqrt().unsqueeze(1)
