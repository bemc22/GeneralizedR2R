...
class ScoreModel(torch.nn.Module):
    ...
    def forward(self, y, physics, update_parameters=False):
        ...
        if self.training:
            ...
            sigma = torch.min(sigma, torch.tensor(1.0, device=y.device))
        else:
            sigma = self.min
        ...
        grad = self.base_model(y_plus, sigma)
        ...
        if noise_class in ["GaussianNoise", "UniformGaussianNoise"]:
            ...
            if self.training:
                ...
                for i in range(J):
                    ...
                    y_plus = y + extra_noise * sigma
                    grad = self.base_model(y_plus, sigma)
                    outi = y + noise_model.sigma**2 * grad
                    outi = torch.clamp(outi, 0, 1)
                    out += outi
            ...
            out = ( y/gain + 1/2 ) * torch.exp( grad )

        elif noise_class == "GammaNoise":
            l = noise_model.l
            # score = self.base_model(y, physics)

            if self.training:
                num = l * y
                denom = (l - 1.0) - y * grad
                # replace close to zero values with 1e-6
                denom = torch.where(denom < 1e-6, torch.tensor(1e-6, device=y.device), denom)
                out   = num / denom
                out   = torch.clamp(out, 0, 1)
            else:
                J = 5
